...
export async function POST(request: NextRequest) {
  try {
    ...
    // Filter and format messages for OpenAI API
    ...
    if (systemPrompt && systemPrompt.trim()) {
      apiMessages.unshift({ role: "system" as "user", content: systemPrompt.trim() });
    }
            ...
            if (line.startsWith('data: ')) {
              ...
              if (dataContent === '[DONE]') {
                ...
                try {
                  ...
                  console.error('Invalid JSON in SSE data from OpenAI:', dataContent);
              ...
              try {
                ...
                console.error('Unexpected data from OpenAI API:', line);
              }
            }
          }
        } catch (error) {
