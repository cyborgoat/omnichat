...
export default function ChatScreen() {
    ...
    const [isUserActivelyScrolling, setIsUserActivelyScrolling] =
        useState(false);
    ...
    const handleSendMessage = async (messageText: string, files?: File[]) => {
        ...
        try {
            ...
            for await (const chunk of chatStream) {
                ...
                while (
                    buffer.includes("__THINKING_START__") &&
                    buffer.includes("__THINKING_END__")
                ) {
                    const startIndex = buffer.indexOf("__THINKING_START__");
                    const endIndex =
                        buffer.indexOf("__THINKING_END__") +
